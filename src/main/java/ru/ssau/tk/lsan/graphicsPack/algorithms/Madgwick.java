...
public class Madgwick implements Algorithm {
    ...
    public void calculatePosition(double[] a, double[] m, double[] g) {
       ...
       /* g[0] -= shift[0];
        ...
        q_est_dot[0] = q_omega_dot[0] ;//- betas[0] * gradient[0];
        q_est_dot[1] = q_omega_dot[1] ;//- betas[1] * gradient[1];
        q_est_dot[2] = q_omega_dot[2] ;//- betas[2] * gradient[2];
        q_est_dot[3] = q_omega_dot[3] ;//- betas[3] * gradient[3];
    ...
    double[] calculateBeta(double[] meanOmega) {
        double w0, wx, wy, wz, k;
        k = Math.PI / 180 * 0.07;
        w0 = meanOmega[0] * k;
        wx = meanOmega[1] * k;
        wy = meanOmega[2] * k;
        wz = meanOmega[3] * k;
        return new double[]{w0, wx, wy, wz};
    }
}
